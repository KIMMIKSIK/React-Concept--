...
### 들어가기전 리액트의탄생 대한 생각 정리
ajax의 도입과 자바스크립트를 통한 동적인 움직임으로 페이지 전체를 다시 렌더링 하기보다 부분적으로 내용을 변경하는 것(리렌더링)이 가능해졌다.(페이지 전체가 깜빡거리고 다시 로드되는 문제 개선) 하지만 SPA를 통한 CSR의 발전을 거듭하며 동적으로 변화해야 될 양이 너무 많아졌고 그 결과 잦은 리렌더링을 통해 속도와 성능이 떨어지게 되었다.
